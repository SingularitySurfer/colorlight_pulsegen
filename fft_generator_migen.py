...
class Fft(Module):
    ...
    def __init__(self, n=32, ifft=False, width_i=16, width_o=16, width_int=16,
        ...
        # Data Memories
        init = [0] * n
        #init[(n//4)] = -(2**(width_i-3))+1000
        #init[(n // 4)] = (2 ** (width_i - 3)) - 1000
        #init[(n // 16)] = 1000#(2 ** (width_i - 3)) - 1000
    ...
    def _bfl_pipe3_dsp_opt(self, ar, ai, br, bi, wr, wi, s):
        ...
        self.PIPE_DELAY = 8
        bias = (1 << self.w_p - 1) - 1
        cr = Signal((self.width_int+1, True), reset_less=True)
        ci = Signal((self.width_int+1, True), reset_less=True)
        dr = Signal((self.width_int+1, True), reset_less=True)
        di = Signal((self.width_int+1, True), reset_less=True)
        ...
        m = [Signal((self.width_int * 2 + 2, True), reset_less=True)
             for _ in range(8)]
        self.sync += [
            ...
            m[6].eq(m[4] - m[2]),  # 6
            m[7].eq(m[5] - m[3]),  # 6
            cr.eq((ar_reg[5] + (m[6] >> (self.w_p+1))) >> s),  # 7
            ci.eq((ai_reg[5] + (m[7] >> (self.w_p+1))) >> s),  # 7
            dr.eq((ar_reg[5] - (m[6] >> (self.w_p+1))) >> s),  # 7
            di.eq((ai_reg[5] - (m[7] >> (self.w_p+1))) >> s),  # 7
        ]
        return cr, ci, dr, di
    ...
    def _twiddle_mem_gen(self, w_idx):
        ...
        init = [(-n & 0xff) for n in range(16)]
        mem = Memory(0xff, 0xff, init=init, name="test")
        wram = Memory((self.width_wram * 2), self.n // 4, init=twiddles, name="twiddle")
...
if __name__ == "__main__":
    test = Fft(n=128, ifft=True, width_int=18, width_wram=18)
    run_simulation(test, test.io_tb(), vcd_name="fft.vcd")
